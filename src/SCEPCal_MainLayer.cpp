...
#include "SCEPCal_MainSegmentationHandle.h"
...
static dd4hep::Ref_t
create_detector_SCEPCal_MainLayer(dd4hep::Detector &theDetector,xml_h xmlElement,dd4hep::SensitiveDetector sens) {
  ...
  auto segmentation=dynamic_cast<dd4hep::DDSegmentation::SCEPCal_MainSegmentation *>(_geoSeg->segmentation());
  ...
  auto CreateEightPointShapeVolume_SetVolAttributes_Place_SetCellId = [
    ...
    std::string volName,
    double dz,
    const std::array<double, 16> vertices,
    xml_comp_t compXml,
  ...
  auto bilinearInterpolateTower = [&](const double* vf, double u, double v) {
    ...
    double y0 =(1-u)*(1-v)*vf[1]
              +    u*(1-v)*vf[3]
              +    u*   v *vf[5]
              +(1-u)*   v *vf[7];
    
    double x1 =(1-u)*(1-v)*vf[8]
              +    u*(1-v)*vf[10]
              +    u*   v *vf[12]
              +(1-u)*   v *vf[14];

    double y1 =(1-u)*(1-v)*vf[9]
              +    u*(1-v)*vf[11]
              +    u*   v *vf[13]
              +(1-u)*   v *vf[15];
  ...
  for (int iPhi=CONSTRUCT_BARREL? BARREL_PHI_START:BARREL_PHI_END; iPhi<BARREL_PHI_END; iPhi++) {
    ...
    dd4hep::Polyhedra barrelPhiAssemblyShape(1,-D_PHI_GLOBAL/2,D_PHI_GLOBAL,
      zBarrelPolyhedra,rminBarrelPolyhedra,rmaxBarrelPolyhedra);
    ...
    for (int iTheta=0; iTheta<N_THETA_BARREL; iTheta++) {
      ...
      dd4hep::Volume barrelThetaAssemblyVolume("barrelThetaAssembly", barrelThetaAssemblyShape, theDetector.material("Vacuum"));
      ...
      for (int nGamma=0; nGamma<N_GAMMA_BARREL; nGamma++) {
        ...
        double projOffsetXmax=std::min(r0e*tan(N_GAMMA_BARREL%2==0?
          D_GAMMA_BARREL/2:D_GAMMA_BARREL/2),PROJ_OFFSET_X);
        ...
        if(N_GAMMA_BARREL%2==0){
          ...
          if (nGamma==(N_GAMMA_BARREL/2-1))  {left=1;right=0;}
          if (nGamma==N_GAMMA_BARREL/2    )  {left=0;right=-1;}
          if (nGamma >N_GAMMA_BARREL/2    )  {left=-1;right=-1;}
        }
        else{
          if (nGamma <N_GAMMA_BARREL/2    )  {left=1;right=1;}
          if (nGamma==N_GAMMA_BARREL/2    )  {left=1;right=-1;}
        ...
        if (nGamma==0)                {left=0;}
        if (nGamma==N_GAMMA_BARREL-1) {right=0;}
        ...
        for(int i = 0; i < XTAL_DIV_R; i++) {
          for(int j = 0; j < XTAL_DIV_R; j++) {
            ...
            CreateEightPointShapeVolume_SetVolAttributes_Place_SetCellId(
              ...
              barrelThetaAssemblyVolume,
              BARREL_SYSTEM_NO, iPhi, N_THETA_ENDCAP+iTheta, nGamma, nEpsilon, 1,
              posGlobal
            );
            numCrystalsBarrel+=1;
          }
  ...
  for (int iPhi=CONSTRUCT_ENDCAP? ENDCAP_PHI_START:ENDCAP_PHI_END;iPhi<ENDCAP_PHI_END;iPhi++) {
    ...
    for (int iTheta=ENDCAP_THETA_START; iTheta<N_THETA_ENDCAP; iTheta++) {
      ...
      for (int nGamma=0;nGamma<nGammaEndcap;nGamma++) {
        ...
        double projOffsetXmax=std::min(r0e*tan(nGammaEndcap%2==0?
          dGammaEndcap/2:dGammaEndcap/2),PROJ_OFFSET_X);
        ...
        if(nGammaEndcap%2==0){
          ...
          if (nGamma==(nGammaEndcap/2-1))  {left=1;right=0;}
          if (nGamma==nGammaEndcap/2    )  {left=0;right=-1;}
          if (nGamma >nGammaEndcap/2    )  {left=-1;right=-1;}
        }
        else{
          if (nGamma <nGammaEndcap/2    )  {left=1;right=1;}
          if (nGamma==nGammaEndcap/2    )  {left=1;right=-1;}
        ...
        if (nGamma==0)                 {left=0;}
        if (nGamma==nGammaEndcap-1)    {right=0;}
        ...
        for(int i = 0; i < XTAL_DIV_F; i++) {
          for(int j = 0; j < XTAL_DIV_F; j++) {
            ...
            CreateEightPointShapeVolume_SetVolAttributes_Place_SetCellId(
              ...
              Transform3D(dispFsub),
              endcapThetaAssemblyVolume,
              ENDCAP_SYSTEM_NO, iPhi, iTheta, nGamma, nEpsilon, 0,
              posGlobal
            );
            numCrystalsEndcap+=1;
        ...
        for(int i = 0; i < XTAL_DIV_R; i++) {
          for(int j = 0; j < XTAL_DIV_R; j++) {
            ...
            CreateEightPointShapeVolume_SetVolAttributes_Place_SetCellId(
              ...
              Transform3D(dispRsub),
              endcapThetaAssemblyVolume,
              ENDCAP_SYSTEM_NO, iPhi, iTheta, nGamma, nEpsilon, 1,
              posGlobal
            );
            numCrystalsEndcap+=1;
